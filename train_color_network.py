...
loss_delta = tf.reduce_mean(tf.abs(estimate_low_wo_C - estimate_high))
...
train_op_delta = optimizer.minimize(loss_delta, var_list=var_Decom_C)
...
eval_every_epoch = 50
train_phase = 'color shift estimation'
numBatch = len(train_low_data) // int(batch_size)

print('loaded ' + './checkpoint/decom/')
saver_I = tf.train.Saver(var_list=var_Decom_I)
ckpt_illu = tf.train.get_checkpoint_state('./checkpoint/decom/')
saver_I.restore(sess, ckpt_illu.model_checkpoint_path)

checkpoint_dir = './checkpoint/color_net/'
...
if ckpt:
	...
	print(iter_num)
	saver1 = tf.train.Saver(var_list=var_Decom_I + var_Decom_C)
	saver1.restore(sess, ckpt.model_checkpoint_path)
else:
	print('No color net pretrained model!')
	iter_num = 0

'''ablation'''
writer = tf.compat.v1.summary.FileWriter("logs/color_net/", sess.graph)
...
loss1 = tf.compat.v1.summary.scalar('loss_delta',  loss_delta)

merge_summary = tf.compat.v1.summary.merge(
	[input_low_tb, input_high_tb, mask_tb, p_R_low_tb, R_high_tb, I_low_tb, I_high_tb, R_low_wo_C_tb, loss1])

saver = tf.train.Saver(var_list=var_Decom_I + var_Decom_C, max_to_keep=5)
...
for epoch in range(start_epoch, epoch):
	for batch_id in range(start_step, numBatch):
		...
		_, est_low, loss = sess.run([train_op_delta, estimate_low1, train_loss], feed_dict={input_low: batch_input_low, input_high: batch_input_high, \
															  lr: learning_rate})
	...
	if (epoch+1) % eval_every_epoch == 0:
		...
		for idx in range(len(eval_low_data)):
			...
			result_0, result_1, result_2 = sess.run([input_low, p_R_low, p_R_low_wo_C], feed_dict={input_low: input_low_eval})
			result_low = np.concatenate([result_0[0:1, :, :, :], result_1[0:1, :, :, :], result_2[0:1, :, :, :]], axis = 2)

			input_high_eval = np.expand_dims(eval_high_data[idx], axis=0)
			input_high_eval = np.tile(input_high_eval, [batch_size, 1, 1, 1])
			result_00, result_11, result_22= sess.run([input_high, p_R_high, p_R_high_wo_C], feed_dict={input_high: input_high_eval})
			result_high = np.concatenate(
				[result_00[0:1, :, :, :], result_11[0:1, :, :, :], result_22[0:1, :, :, :]], axis=2)
